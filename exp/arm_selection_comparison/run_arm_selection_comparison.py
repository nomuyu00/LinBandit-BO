#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Arm Selection Method Comparison Experiment.

reward_function_comparison の枠組みに準拠しつつ、LinBandit-BO の
「アーム（方向）の選択方式」だけを差し替えて比較する。

比較対象（勾配報酬は共通: r_vec = |∇μ| / max(L_hat, L_min)）
- Discrete(arms):   既存実装（座標+ランダムの有限集合から LinUCB）
- Continuous-EVD:   信頼楕円体の原点から最遠点の方向（固有分解+二分探索）
- Continuous-Fixed: その高速近似（固定点反復）

出力（図/CSV/NPY）のフォーマットは reward_function_comparison と同一構成。
"""
...
OUTPUT_ROOT = os.path.join(
    os.path.dirname(__file__), "output_results_arm_selection_comparison"
)
...
class ExperimentConfig:
    """アーム選択方式の比較設定（reward_function_comparison と互換のキー）。"""
    ...
    n_runs: int = 20
    L_min: float = 0.1
    l_min_floor: float = 0.10  # RBF の lengthscale 下限（正規化空間）
    ...
    def __post_init__(self) -> None:
        # 互換環境変数
        runs = os.getenv("RF_RUNS") or os.getenv("AS_RUNS")
        if runs is not None:
            self.n_runs = int(runs)
        n_iters = os.getenv("RF_ITERS") or os.getenv("AS_ITERS")
...
class ArmSelectionLinBanditBO:
    """LinBandit-BO（アーム選択方式のみ差し替え、勾配報酬は共通）。

    arm_mode:
      - "Discrete(arms)":   既存方式（座標+ランダムの有限集合→ LinUCB）
      - "Continuous-EVD":   楕円体最遠点方向（固有分解+二分探索）
      - "Continuous-Fixed": 固定点反復の近似
    共通: GP 入力正規化 + RBF(ARD) + lengthscale の下限 l_min_floor。
    """
    ...
    def __init__(
        ...
        self.X = self.X.to(dtype=torch.get_default_dtype())
        ...
        # 計測
        self.reward_history: List[np.ndarray] = []  # 次元別にログ（非負値）
    ...
    def optimize(self) -> None:
        print(f"ArmSelection-BO: {self.dim}D, mode={self.arm_mode}, max {self.n_max}")
        self.initialize()
        ...
        while n_iter < self.n_max:
            ...
            reward_vec = grad_vec.abs()

            grad_norm = float(reward_vec.norm().item())
            if grad_norm > self.L_hat:
                self.L_hat = grad_norm
            L_eff = max(self.L_hat, self.L_min)
            scaled_reward_vec = reward_vec / L_eff

            # 線形バンディット更新
            x_arm = direction.view(-1, 1)
            self.A += x_arm @ x_arm.t()
            self.b += scaled_reward_vec

            # ログ
            self.reward_history.append(scaled_reward_vec.detach().cpu().numpy())
...
def run_compare_for_function(
    ...
    for run_idx in range(cfg.n_runs):
        ...
        for label in cfg.arm_algs:
            torch.manual_seed(seed)
            np.random.seed(seed)
            optimizer = ArmSelectionLinBanditBO(
...
def plot_convergence(func_name: str, results: Dict[str, List[Dict[str, object]]], cfg: ExperimentConfig) -> None:
    ...
    for runs in results.values():
        ...
        if final_thetas:
            ...
            theta_means.append(np.zeros(cfg.dim))
...
def plot_diagnostics(func_name: str, results: Dict[str, List[Dict[str, object]]], cfg: ExperimentConfig) -> None:
    ...
    # 方向コサイン類似度の分布（EVD vs Fixed vs Discrete）
    ax = axes[2]
    # 簡易に方向ノルムの分布だけ可視化（詳細は省略）
    dir_norm_rows = []
    for alg, runs in results.items():
        for run in runs:
